...
pnpm quality-gate        # READ-ONLY verify, parallel: build + turbo(lint+format+typecheck) + test + check:exports + check:bank-paths + check:bank-tokens + check:adrs + check:portable + boundaries + lint:ws + deps:lint + test:policy + gitleaks
```
