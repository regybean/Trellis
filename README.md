...
- **Rows prove subsetting.** The slim apps **drop auth and Stripe entirely** — they don't depend on `@acme/auth`, `@acme/billing`, or `@acme/subscriptions`, and inject a constant local principal + `unlimitedEntitlements` instead ([ADR 0010](apps/docs/adr/0001-slim-no-auth-apps.md)). A no-auth/no-billing product is _a different subset of the same packages_, not a fork — and the build enforces it (the dependency graph literally doesn't contain the auth provider or Stripe for a slim app).

That row is only possible because billing and auth are **injected, not imported**: the platform substrate depends on neutral contracts, and the app chooses the implementation ([auth seam ADR 0003](packages/shared/auth/docs/adr/0003-framework-agnostic-auth-seam.md), [entitlements seam ADR 0006](packages/platform/entitlements/docs/adr/0001-entitlements-injection-seam.md)).
...
A repo that starts from Trellis normally starts by copying it, and from that moment every fix made on either side stays there. The bank is the alternative: a consumer repo pins a `bank/YYYY-MM-DD` tag in `bank.manifest.json` and names the packages it wants, and `pnpm bank:sync` rewrites a local `vendor/trellis` branch to hold Trellis filtered down to what that selection covers at the pinned ref. Because each sync parents on the last one, `git merge vendor/trellis` is an ordinary three-way merge: upstream changes replay, local edits survive, and conflicts appear only where both sides touched the same lines ([ADR 0037](docs/adr/0037-vendored-git-subset-three-way-merge.md)).
...
- **Nobody enumerates paths** ([ADR 0039](docs/adr/0039-the-selection-is-the-contract.md)). What's on offer is every workspace package the `pnpm-workspace.yaml` globs match, so it can't go stale; [`bank.paths.json`](bank.paths.json) carries only what no derivation gives you — six bundles for content that can't be a package, and the exclusions with a reason each. Never an app, a README or a lockfile: those are consumer identity. A manifest names packages and bundles, and the sync resolves the transitive workspace closure at the ref.
- **Drift is consumer-side**, because Trellis can't see its consumers. `pnpm bank:sync --check` reports how far behind the pinned tag is, which paths would enter or leave the closure on a bump, and which vendored paths the consumer has modified, with distinct exit codes to gate CI on.
...
| The packages, in the app you have | `node tooling/bank/src/setup-wizard.mjs`, then `bank:sync`, then wire each package in by reading its `ADAPTER.md`      | Your app stays yours; you take the rails under it       |
| A subset, in an app you write     | `node tooling/bank/src/setup-wizard.mjs` into an empty repo, then `bank:sync`, then write the app against `ADAPTER.md` | Nothing enumerates the closure for you; the wizard does |
...
Full agent brief in [AGENTS.md](AGENTS.md); workflow details in [docs/agents/](docs/agents/).
...
- **Auth is self-hosted, and thin.** The full apps run [Better Auth](packages/shared/auth/docs/adr/0001-self-hosted-better-auth.md) against their own Postgres, email and password only. There is no social provider (adding one is config plus a client secret — the `account` table already carries the OAuth columns) and no email transport, so password reset and email verification aren't wired. Roles are a column on the user row, and the first admin is seeded with a SQL update ([Getting started](docs/getting-started.md#auth-better-auth-secret-required-for-the-full-apps)). Self-hosting replaced a hosted identity provider, and **there is no migration path off that provider**: a template starting fresh loses nothing, but a deployment with real users needs its own plan for moving identities and credentials.
- **Each app owns its env; there is no shared root `.env`.** The application env surface used to live in a shared root `.env` layered under every app; it was deprecated so each app declares its full env (`apps/<app>/.env`), duplicating shared model-provider secrets by design ([ADR 0029](apps/docs/adr/0003-per-app-env-ownership.md)). Only `deploy/.env` (infra container passwords) remains at the root. `SECRET_MAP` in `secrets.config.sh` maps all four apps + infra for the pluggable secrets sync ([ADR 0001](tooling/secrets-sync/docs/adr/0001-pluggable-secrets-sync.md); opt-in via `SECRETS_BACKEND`, `localstack` for dev or `aws` for a real vault).
- **Model providers are settling.** Selection lives in [`@acme/models`](packages/shared/models/CONTEXT.md) — `bedrock` / `openrouter` / `ollama` by env ([@acme/models ADR 0001](packages/shared/models/docs/adr/0001-multi-provider-models.md)). Ollama is the default so the repo runs with no credentials; dev models are tiny/CPU-only, not production quality.
