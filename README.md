...
A repo that starts from Trellis normally starts by copying it, and from that moment every fix made on either side stays there. The bank is the alternative: a consumer repo pins a `bank/YYYY-MM-DD` tag in `bank.manifest.json` and names the packages it wants, and `pnpm bank:sync` rewrites a local `vendor/bank` branch to hold Trellis filtered down to what that selection covers at the pinned ref. Because each sync parents on the last one, `git merge vendor/bank` is an ordinary three-way merge: upstream changes replay, local edits survive, and conflicts appear only where both sides touched the same lines ([ADR 0037](docs/adr/0037-vendored-git-subset-three-way-merge.md)).
