...
- **Clerk is a hard dependency for the _full_ apps.** The _framework_ is abstracted behind the auth seam ([ADR 0003](docs/adr/0003-framework-agnostic-auth-seam.md)), but the _provider_ isn't: `@acme/auth` re-exports `@clerk/clerk-react` hooks/components that features import directly (e.g. `UserButton`, billing's `useAuth`), and the full apps sharpen the injected session user to a backend Clerk `User`. The tRPC session type itself is provider-neutral ([ADR 0003 amendment](docs/adr/0003-framework-agnostic-auth-seam.md)). So `nextjs`/`tanstack-start` need Clerk env. The **slim apps sidestep it entirely** (no `@acme/auth`, constant local principal) — fully decoupling the provider from the _full_ apps is the remaining work.
