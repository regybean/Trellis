...
- **Not built yet:** the setup wizard — now just a picker over the derived package list that writes `packages` and `bundles`. No `bank/*` tag has been cut, so there is nothing to pin against today, and no consumer has contributed anything back.
