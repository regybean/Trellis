...
- **Setup is scripted, not yet interactive.** `node scripts/setup-wizard.mjs --upstream <url> --ref <tag> --packages <names> --bundles <names>` resolves that selection against the bank at that ref and writes `bank.manifest.json`, refusing a name that doesn't exist there rather than leaving it for the first sync to hit. It copies nothing — `bank:sync` stays the only thing that moves files.
- **Not built yet:** the picker over the derived package list, so the selection is arguments rather than a menu today. No `bank/*` tag has been cut, so there is nothing to pin against, and no consumer has contributed anything back.
