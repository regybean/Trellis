...
- **Clerk is a hard dependency for the _full_ apps.** The _framework_ is abstracted behind the auth seam ([ADR 0003](docs/adr/0003-framework-agnostic-auth-seam.md)), but the _provider_ isn't: `@acme/auth` re-exports `@clerk/clerk-react` hooks/components that features import directly (e.g. `UserButton`, billing's `useAuth`). The tRPC session type is provider-neutral ([#220](https://github.com/regybean/Trellis/issues/220)) — the apps map Clerk onto it — but the client-side surface still binds to Clerk. So `nextjs`/`tanstack-start` need Clerk env. The **slim apps sidestep it entirely** (no `@acme/auth`, constant local principal) — fully decoupling the provider from the _full_ apps is the remaining work.
