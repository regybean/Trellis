/**
 * Composition root — @acme/tanstack-slim.
 *
 * Everything this app injects into a seam is constructed here, once, and every
 * entry point imports the result: the tRPC route seam (`src/lib/trpc-route.ts`)
 * and the generation worker (`worker.ts`). Construction lives in `src/server/`;
 * request-time resolution stays in `src/lib/` and imports across.
 *
