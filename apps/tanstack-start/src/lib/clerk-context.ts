 ...
 * and shape it into the fields the neutral tRPC context expects. Each feature's
 * `createTRPCContext` (re-exported from the platform seam) consumes this — the
 * feature packages never import a framework Clerk SDK themselves.
 *
...
export async function resolveClerkContext(req: Request) {
  ...
  return { headers: req.headers, req, auth: authObject, user };
}
