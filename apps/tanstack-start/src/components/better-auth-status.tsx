 ...
 *
 * `initialUserId` is the id `__root`'s `beforeLoad` already resolved on the
 * server. Better Auth's client fetches the session itself, so without a seed the
 * first client render would report "still loading" even for a signed-in visitor
 ...
 * the second.
 *
 * The mapping is duplicated in `apps/nextjs` rather than shared, and that is the
 * seam working as designed: `authClient` is app-owned (each app builds its own),
 * and a shared component would have to pull `better-auth` into the substrate —
 * which the slim, no-auth apps must never see (ADR 0010).
 */
