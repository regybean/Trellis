 ...
 * feature's QueryClient singleton is created — a client `useAuth()` would resolve
 * too late and the singleton would attach no persister. `scopeKey` scopes each
 * user's cache and makes a different user or a new deploy discard the prior
 ...
 * the id arrives as a plain string. Mirrors the Next.js app's adapter — same
 * conceptual seam on a second framework.
 */
...
function ClearCacheOnLogout({
  ...
  useClearCacheOnLogout(Boolean(userId), clearStore);
  return null;
}
