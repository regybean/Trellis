/**
 * Composition root — @acme/tanstack-start.
 *
 * Everything this app injects into a seam is constructed here, once, and every
 * entry point imports the result: the tRPC context resolvers
 * (`src/lib/trpc-context.ts`) and the generation worker (`worker.ts`).
 * Construction lives in `src/server/`; request-time resolution stays in
 * `src/lib/` and imports across.
 *
