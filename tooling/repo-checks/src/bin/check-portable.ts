 ...
 * package, an issue reference or a root ADR naming an app fails `pnpm lint`.
 * It landed report-only first, so the count the rules produced across the whole
 * repo was reviewable — and the rules arguable — before the sweep that cleared
 * it was made in their name.
 *
 * Usage:
 *   tsx src/bin/check-portable.ts [repo-root]
 */
