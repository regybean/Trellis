 ...
 * toasts ([ADR 0001](../docs/adr/0001-notifications-seam.md)). It renders its
 * own tRPC provider and, inside it, a headless always-on tail child — so NO
 * page owns the subscription hook: notifications are chrome, not a page
 * feature. Mount it once per app, adjacent to the ingest tRPC provider; the
 * mount is byte-identical in all 4 apps (no persister, no `scopeKey`, no client
 * principal — the server keys `userId` from `ctx`).
 *
 ...
 * so "does it subscribe when signed in" is not assertable at the HTTP boundary
 * the way "does it stay quiet when signed out" is. This keeps the rule itself
 * directly testable without mocking a seam the feature owns.
 *
 * `null` is an app with no `AuthStatusProvider` — the slim apps, which inject a
 * synthetic session server-side. For them an absent provider means "always
 * authorized", so it must stay enabled rather than read as signed-out and go
 * dark. Everywhere else this waits for a *resolved* signed-in session:
