 ...
 * running Redis — no env mocks. The only behavioural mock is `server-only`,
 * which lets `publish` (guarded by `import 'server-only'`) be imported under
 * vitest. Nothing else is mocked — the whole point is the real round-trip
 * through Redis.
 */
