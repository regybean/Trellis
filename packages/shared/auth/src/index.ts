 ...
 *
 * There is nothing else to export because Better Auth ships no UI. The *app*
 * owns the client (`createAuthClient`), `@acme/ui` owns the presentation, and
 * `@acme/hooks` owns the client-side status seam — so this package ships no React
 * at all, and the slim apps' graph never sees an auth provider (ADR 0010).
 *
