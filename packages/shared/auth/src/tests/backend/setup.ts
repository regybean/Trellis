 ...
 * validate against the real running service rather than a mock. The four Better
 * Auth tables are provisioned once by the global `drizzle-kit push --force`,
 * which creates them in the `auth` schema because the canonical app re-exports
 * them.
 *
