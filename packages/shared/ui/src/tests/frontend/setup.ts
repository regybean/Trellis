...
// this suite needs no MSW server (the HTTP-boundary fake has nothing to
...
// plain `setup.ts` rather than the feature recipe's `setup.tsx`
// (docs/TESTING.md § "Setup and config") — there is no provider tree to wrap,
// so no JSX.
