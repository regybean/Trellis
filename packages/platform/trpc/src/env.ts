...
/**
 * The tRPC substrate's environment — one key, and it is a runtime mode rather
 * than a tunable: `withTimingLog` adds an artificial 100-500ms stall in
 * development so local UIs actually render their loading states.
 *
 ...
 * private `_config` to learn it. Validating it here instead keeps the middleware
 * bodies taking only what they log or decide on, and keeps the read off
 * `process.env` in feature code, where the lint rule rightly bans it.
 *
