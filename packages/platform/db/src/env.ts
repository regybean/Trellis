 ...
 * edge. Each slice resolves the same selector at its own edge, so the profiles
 * agree without threading a context.
 */
const appEnv = resolveAppEnv(process.env.APP_ENV);

/**
 * The resolved Postgres connection, declared once. Read by `createDb()` and the
 * rag storage/vector clients.
 *
 ...
 * `runtimeEnv`, so every key is env-overridable; `DB_HOST` / `DB_PORT` are the two
 * that *must* be — a testcontainer hands back a mapped port and a prod endpoint is
 * infra-injected, so no profile can know them. That used to be hand-rolled here as
 * `process.env.DB_HOST ?? config.DB_HOST` and `Number(process.env.DB_PORT)`;
 * coercion now lives in the schema, so `DB_PORT=abc` fails loudly instead of
 * reaching a caller as `NaN`.
 *
...
export const env = createEnv({
  ...
  runtimeEnv: {
    ...
    // collect page data. Stub it in that case so construction succeeds;
