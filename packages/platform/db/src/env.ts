...
export const env = createEnv({
  ...
  runtimeEnv: {
    ...
    // which builds with a real `APP_ENV` — see `shouldSkipEnvValidation`) there
    // is no password and none is needed: no query runs. But Mastra's
    // `PgVector`/`PostgresStore` validate a non-empty password in their
    // *constructor*, which the Next build triggers by importing the chat route
    // to collect page data. Stub it in that case so construction succeeds;
    // runtime and vitest never skip, so the real secret is still enforced
    // there.
    DB_PASSWORD:
      readEnv('DB_PASSWORD') ??
