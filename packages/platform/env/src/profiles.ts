 ...
 * wrong literal is a compile error, not just a runtime one ([ADR 0001](../docs/adr/0001-one-env-factory-per-slice.md)'s
...
/**
 * The closed profile set ([ADR 0001](../docs/adr/0001-one-env-factory-per-slice.md) §2): `default` *is* `development`; `staging`
 ...
 * rule, not an oversight: every key is env-overridable ([ADR 0001](../docs/adr/0001-one-env-factory-per-slice.md) §4), so a
 ...
 * whether the resolved profile supplies a value ([ADR 0001](../docs/adr/0001-one-env-factory-per-slice.md) §1):
 *
 ...
 *   config default alongside the secrets ([ADR 0001](../docs/adr/0001-one-env-factory-per-slice.md) §3).
 *
 ...
 * `createFinalSchema` seam ([ADR 0001](../docs/adr/0001-one-env-factory-per-slice.md)) — no fork, no patch:
 *
 * ```ts
 ...
 * every key does ([ADR 0001](../docs/adr/0001-one-env-factory-per-slice.md) §4). Profile values are attached to the schema, so a
 ...
 * per-key relaxation on a run that cannot supply secrets ([ADR 0001](../docs/adr/0001-one-env-factory-per-slice.md) §3) is
