...
export default [
  ...
  // slice's `runtimeEnv` goes through
  // ([ADR 0001](docs/adr/0001-one-env-factory-per-slice.md) §4). It is the same
  // sanctioned edge as an `env.ts`, factored out so the guard against touching
  // `process` in a browser bundle lives in one place. Scoped here, to this file
  // in this package, rather than a repo-wide `**/read-env.ts` ignore: the
  // exemption is for @acme/env being the central env mechanism
  // ([ADR 0001](docs/adr/0001-one-env-factory-per-slice.md)), so no other
