...
/**
 * The billing values the client seam reads, supplied directly here — config is
 * pure, so a test constructs it with no env. The plan IDs match the
 ...
 * exactly how the consuming apps drive it (never mock a seam the feature owns).
 *
...
/**
 * Build the providers every billing frontend test renders under: the app's
 * single QueryClient (a feature provider renders none of its own, so a test has
 * to mount one exactly as an app does), the feature's tRPC provider, the
 * `BillingConfigProvider` carrying the client config + server-derived
 * localstripe mode, and a real `<ToastContainer />` so success/error toasts are
 * asserted as DOM text, not via a mocked `toast`. `localstripeMode` defaults to
 * `false` (real Stripe) — the mode is threaded through the provider seam, so a
 * test opts into localstripe by passing `{ localstripeMode: true }` rather than
 * touching `NODE_ENV`.
 */
