...
**Offline read via the per-query persister** (#216): `IngestTRPCReactProvider` accepts an app-supplied `scopeKey`; when present, ingest's `QueryClient` attaches the shared `@acme/hooks` per-query persister ([ADR 0025](../../../docs/adr/0025-per-query-indexeddb-persister.md)) under its own IndexedDB store `rq-ingest`, so the documents page paints from cache on a cold open instead of skeletons. Only `documents.list` is marked persistable (`meta: persistMeta`) — it is the query that buys the paint. `documents.progressSnapshot` is deliberately **not** marked: it is in-flight Upload state whose whole point is to be read fresh from the retained Stream, and a persisted copy would re-seed the panel with rows that finished hours ago behind a `lastId` the Stream has since expired past. Both queries are pinned to ingest's own client via `useIngestQueryClient` (#82) — apps nest several feature providers, so an unpinned `useQuery` binds to whichever `QueryClientProvider` is innermost and would silently never persist. `maxAge` is 24 hours (`gcTime >= maxAge`) and `buster` is `INGEST_PERSIST_VERSION:scopeKey`. That version is pinned in `trpc/query-client.ts` rather than read from `NEXT_PUBLIC_APP_VERSION` (chat's choice), mirroring feedback: what invalidates an ingest snapshot is a change to the `documents.list` row shape, not every deploy — bump it when that shape changes. `clearIngestPersistedCache()` is exported for the app's logout path (full apps call it alongside `queryClient.clear()`; slim apps have no logout and never do). With no `scopeKey` — or if IndexedDB is unavailable — ingest is network-only, exactly as before.
_Gotcha_: ingest's `staleTime` is **`0`**, and that is load-bearing, not a default left at zero (it replaced a 30s value when ingest opted in). On a cold open the persister _is_ the queryFn: it restores the snapshot, returns it, and only then schedules a background refetch `if (query.isStale())` — a check that reads `staleTime` and ignores `refetchOnMount`. Any `staleTime > 0` therefore serves a restored snapshot **without revalidating**, which on this surface means uploading a Document, reloading, and getting the pre-upload list. That always-firing refetch is also what makes the persister's floating `query.fetch()` reachable, so ingest depends on ADR 0025's pinned `pnpm patch` (`patches/@tanstack__query-persist-client-core@5.90.2.patch`) to swallow a failed background revalidation; the offline case in `use-documents-persistence.test.tsx` fails the run as an unhandled rejection if that patch stops applying.
