...
/**
 * Ingest's environment, declared once — composed into an app's env
 ...
 * (empty) in staging/production. Every one of them is env-overridable, which is
 * what a real deploy needs — a bucket name is per-environment data, not a value
 * worth a commit.
 *
