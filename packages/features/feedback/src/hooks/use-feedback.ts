...
import { useFeedbackQueryClient, useTRPC } from '../trpc/react';
...
export function useFeedback(messageId: string, threadId: string) {
  const trpc = useTRPC();
  // Pin to feedback's own QueryClient (the persister-bearing one), not the
  // nearest provider in context — see useFeedbackQueryClient / #82.
  const queryClient = useFeedbackQueryClient();
