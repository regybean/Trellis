...
// seam as `message_feedback`. It defines a Folder (a user-created grouping of
// Conversations); the Folder *assignment* lives on the Mastra thread as
// `metadata.folderId` (a single scalar, so a Conversation is in at most one
// Folder). There is NO foreign key from the Mastra thread metadata to this
// table: deleting a Folder leaves member threads with a dangling `folderId`
// that simply stops resolving, returning those Conversations to their Date
// Bucket with no per-Conversation write (lazy delete). See
