...
// Also the `gcTime` of every persisted chat query, so an in-memory entry is
// never garbage-collected before its stored copy expires.
const CHAT_MAX_AGE = 7 * 24 * 60 * 60 * 1000; // 7 days

// Chat's client half, assembled from the shared factory (`@acme/hooks`). All
// the scaffold — the `NODE_ENV==='test'` `httpLink` MSW seam, the provider
...
// `/api/trpc/chat`), the file-upload-aware transport, its `stream`
// subscription, and the 7-day per-query persister. The app owns the
