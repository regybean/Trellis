...
The pattern where the assistant retrieves relevant Chunks from the knowledge base before generating a response, grounding the answer in the user's own uploaded Documents. Implemented agentically: the chat Agent is given a Mastra vector-query tool (`@acme/rag`) and decides _when_ to retrieve — but never _what it may see_. Retrieval is bounded per **Turn** by a **Retrieval scope** the server builds, which the agent is not told and cannot author.
_Avoid_: "search", "lookup", "context injection"

**Source Selection**:
The **Data Sources** the user ticked in the composer for this **Turn** (`@acme/rag`'s `SourceSelection`). Per-Turn, not per-Conversation: it travels on `chat.send`, onto the Turn's record, and into the **Generation worker**'s job payload — the worker never reads a selection off the thread. Zero or more; the empty set is the default, is sticky, and means _retrieve nothing_. `chat.send` narrows it to the Sources the caller actually owns and **drops** the rest with a log line rather than rejecting the send — dropping is rag's policy for a retrieval scope, on the grounds that Source rows are hard-deleted, so "never yours" and "yours until a second ago" are indistinguishable and ordinary staleness must not cost the user their message. The narrowing also resolves each surviving Source's **name**, because that is what the Turn's **Sources included** receipt records.
Stickiness is **derived, not stored**: a new Conversation starts empty, a fresh mount reads the latest receipt for the thread rather than client state, and the selection is intersected with the Sources the caller still owns every time `dataSources.list` settles — so one deleted elsewhere unticks itself. Deliberately **not** kept on thread metadata the way `folderId` is; that would be a second write path holding a privacy-relevant value the worker must not trust.
_Avoid_: "filter" (the server derives that, never the client), "allowed sources", treating the raw request array as the scope

**Retrieval scope**:
What a Source Selection resolves into when the Turn actually runs — rag's trusted `RequestContext`, built inside `streamScopedTurn` and passed to the agent untouched. It is narrowed **again** here, so a Source deleted between the send and the run falls out and the Turn answers from what is left. Its `hasSources` drives `activeTools`: an empty scope offers the agent no retrieval tool at all, so the Turn costs no query embedding and no scan. The scope is the validated fact; the Selection is only the request ([ADR 0009](docs/adr/0009-per-turn-retrieval-scope.md)).
_Avoid_: conflating it with the Selection; recomputing `hasSources` from the raw array

**Sources included**:
What a settled **Turn** discloses about its **Source Selection** — the names of the Data Sources the Turn was scoped to, shown beneath the assistant Message. Persisted per Message as a receipt (`message_data_source`, an **app-owned table** keyed on the assistant `message_id`) and written by the **Generation worker**, because Mastra mints that id during streaming; the worker writes the already-validated set from its job payload rather than reading a selection back off anything. The receipt carries each Source's **name** beside its id, so it still reads correctly once the Source is deleted or renamed — it states what was included, under the name it had. The empty collection is a **value**, not an absence: "asked with no Sources" stays distinguishable from a Message with no receipt at all, which is what the sticky empty set depends on. A **failed Turn writes no receipt**, so stickiness reads through to the previous settled Turn instead of resetting.
The label is _included_, never _used_: the agent decides whether to call the retrieval tool at all, and true citation is out of scope.
_Avoid_: "citations", "sources used", resolving the names from `dataSources.list` on read
...
- A **Turn** is triggered by `chat.send({ query, conversationId, turnId, dataSourceIds })` (a mutation): the ownership builder asserts ownership, `chat.send` narrows the **Source Selection** to the caller's own Sources, then `chat.send` delegates the begin sequence to the Turn lifecycle's **`beginTurn`** — take the **In-flight lock** (`SET NX`, valued by `turnId`) first, discard a stale Stream, ensure the Conversation, persist the **user** Message, run `chat.send`'s credit gate + consume (passed in as a closure so it stays inline), and enqueue a **Generation worker** job — then returns. `beginTurn` owns the ordering and unwinds the lock on any failure; `chat.send` keeps only the credit consume inline. It never streams. Two-tab races return a discriminated result: `{ accepted, turnId }` for the winner, `{ alreadyInflight }` for the loser (which rolls back, attaches, and restores its draft).
- The worker resolves the Turn's **Retrieval scope** inside `streamScopedTurn`, which is the only `chatAgent.stream` call site in the package (lint-enforced). rag narrows the Selection a second time there, so the scope reflects what is still the caller's when the Turn actually runs — not what was true at send.
- The worker also writes the Turn's **Sources included** receipt, in the same step that persists the assistant Message and before it emits the terminal, so a client invalidating on the terminal cannot beat the write. Only the paths that persist a Message write one, which is what makes a failed Turn invisible to stickiness. The write is fail-soft: by then the answer exists and has been paid for, so a receipt failure is logged and swallowed rather than turning a working Turn into a refunded error.
