...
| [@acme/rag ADR 0001](../packages/shared/rag/docs/adr/0001-mastra-rag-and-memory.md)                    | Mastra owns RAG + Memory; Drizzle mirrors are query-only read models.                          |
...
| [0003 (models)](../packages/shared/models/docs/adr/0001-multi-provider-models.md)                      | Multi-provider models behind a single `@acme/models` package.                                  |
| [@acme/billing ADR 0001](../packages/features/billing/docs/adr/0001-localstripe-dev-billing.md)        | localstripe for dependency-free local-dev billing.                                             |
...
| [@acme/chat ADR 0005](../packages/features/chat/docs/adr/0005-folder-storage-split.md)                 | Conversation Folders: split storage, lazy delete.                                              |
| [@acme/ui ADR 0001](../packages/shared/ui/docs/adr/0001-admin-user-widgets-to-ui.md)                   | Framework-agnostic admin user widgets belong in `@acme/ui`, not duplicated per app.            |
...
| [@acme/db ADR 0001](../packages/platform/db/docs/adr/0001-db-connection-platform-package.md)           | The Postgres connection lives in `@acme/db`, peer to `@acme/redis`.                            |
...
| [@acme/hooks ADR 0001](../packages/shared/hooks/docs/adr/0001-per-query-indexeddb-persister.md)        | A per-query IndexedDB persister gives chat + feedback an offline read.                         |
...
| [@acme/notifications ADR 0001](../packages/shared/notifications/docs/adr/0001-notifications-seam.md)   | `@acme/notifications` is a shared package that owns a tRPC router.                             |
| [@acme/ingest ADR 0001](../packages/features/ingest/docs/adr/0001-ingest-progress-survives-refresh.md) | Ingest progress survives a refresh: snapshot, then resume from `lastId`.                       |
| [@acme/redis ADR 0001](../packages/platform/redis/docs/adr/0001-durable-redis-stream-primitive.md)     | One durable Redis-stream primitive behind chat / ingest / notifications.                       |
| [0033 (env)](../packages/platform/env/docs/adr/0001-one-env-factory-per-slice.md)                      | One env factory per slice; profiles ride `createFinalSchema` and every key is overridable.     |
| [0033 (forms)](../packages/shared/ui/docs/adr/0002-tanstack-form-in-acme-ui.md)                        | TanStack Form is the form library, and `@acme/ui` is where it lands.                           |
| [0034 (auth)](../packages/shared/auth/docs/adr/0001-self-hosted-better-auth.md)                        | Auth is self-hosted Better Auth, with sessions as rows in Postgres.                            |
...
| [@acme/auth ADR 0002](../packages/shared/auth/docs/adr/0002-auth-tables-in-a-dedicated-schema.md)      | Auth tables live in a dedicated `auth` schema, outside the per-app one.                        |
