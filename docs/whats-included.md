...
pnpm quality-gate            # then verify (read-only, parallel): build + turbo(lint+format+typecheck) + test + check:exports + check:bank-paths + check:bank-tokens + check:adrs + check:portable + boundaries + lint:ws + deps:lint + test:policy + gitleaks
```
...
| `pnpm bank:sync`                                           | **Consumer-side.** Rewrite `vendor/trellis` to the bank subset pinned in `bank.manifest.json`; merges nothing ([the bank](bank.md))                                                                                                                                                                  |
...
| `pnpm quality-gate`                                        | The full pre-push gate — **read-only, parallel**: build + turbo(lint+format+typecheck) + test + check:exports + check:bank-paths + check:bank-tokens + check:adrs + check:portable + boundaries + test-policy + lint:ws + deps:lint + gitleaks                                                       |
