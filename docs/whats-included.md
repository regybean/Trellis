...
The **columns** prove portability — the same slices under two frameworks, differing only in the per-app adapter. The **rows** prove subsetting — the slim apps drop auth and Stripe _from the dependency graph_ (no `@acme/auth`, `@acme/billing`, `@acme/subscriptions`), injecting a constant local principal + `unlimitedEntitlements` instead ([ADR 0010](../apps/docs/adr/0001-slim-no-auth-apps.md)). A no-auth/no-billing product is a _different subset of the same packages_, not a fork.
...
| [`@acme/auth`](../packages/shared/auth/)               | Self-hosted [Better Auth](../packages/shared/auth/docs/adr/0001-self-hosted-better-auth.md) behind a **framework-agnostic seam** — the instance factory, the four tables and the role parse; no React, and the feature never imports an auth SDK. Email + password, sessions as rows, roles on a column. See [ADR 0003 (auth seam)](../packages/shared/auth/docs/adr/0003-framework-agnostic-auth-seam.md). | ✅     |
| [`@acme/hooks`](../packages/shared/hooks/)             | Shared React hooks.                                                                                                                                                                                                                                                                                                                                                                                         | ✅     |
| [`@acme/rag`](../packages/shared/rag/CONTEXT.md)       | RAG + conversation memory on [Mastra](../packages/shared/rag/docs/adr/0001-mastra-rag-and-memory.md): vector store, document uploader, memory storage. Provider-agnostic — models come from `@acme/models`.                                                                                                                                                                                                 | ✅     |
| [`@acme/models`](../packages/shared/models/CONTEXT.md) | Resolves chat + embed AI-SDK models from an env-selected provider (`bedrock` / `openrouter` / `ollama`). **Ollama is the default, so the repo runs with no cloud credentials.** See [@acme/models ADR 0001 (multi-provider)](../packages/shared/models/docs/adr/0001-multi-provider-models.md).                                                                                                             | ✅     |
...
| Package                                                                | What it does                                                                                                                                                     | Status |
| ---------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| [`@acme/trpc`](../packages/platform/trpc/CONTEXT.md)                   | The single tRPC init + request-pipeline middleware every feature reuses: auth, OTel tracing, timing, rate limiting. Owns _how_ a request is handled, not _what_. | ✅     |
...
| [`@acme/secrets-sync`](../tooling/secrets-sync/) | The env compose/diff/merge behind `env:pull` / `env:push` |
...
- **Eject any app into a standalone repo** — `pnpm prune <app>` ([scripts/extract-app.sh](../scripts/extract-app.sh)) runs `turbo prune` + a config overlay to emit a self-contained single-app repo under `out/` that installs and builds on its own (`pnpm install`), no monorepo required. The slice contract makes an app extractable; this is the proof. Take this route when you want a Trellis app to start from.
- **Author a bank selection for a repo whose app is yours** — `pnpm setup:wizard` ([tooling/bank/src/setup-wizard.mjs](../tooling/bank/src/setup-wizard.mjs), plain `node tooling/bank/src/setup-wizard.mjs` in a consumer repo before its first sync) opens a keyboard-driven picker over every package the bank offers at a ref, groups it by layer, and shows what each choice drags in and why. It writes `bank.manifest.json` and copies nothing; `bank:sync` moves the files ([the bank](bank.md#which-route-you-are-on)). Take this route when the app is already yours, or when you are writing it. `prune` ejects a Trellis app; the wizard subscribes your app to the packages.
- **Graph-derived dev infra** — `pnpm dev [app]` starts only the Docker services its targets need: the **union of `acme.infra` over each app's transitive package closure** ([scripts/resolve-infra.ts](../scripts/resolve-infra.ts)), waits for health, pushes schema, then runs the servers. No `core` always-on set — a slim app derives no Stripe/Redis container because the graph says so ([ADR 0009](../tooling/workspace-graph/docs/adr/0001-graph-derived-dev-infra.md)).
...
- **One identity partitions every datastore** — a single `NEXT_PUBLIC_WEBAPP` value namespaces each app's Postgres schema _and_ Redis keyspace (via an invisible client `Proxy`), so all four apps share one Postgres + one Redis without collisions, fail-loud if unset ([ADR 0008](../packages/platform/redis/docs/adr/0002-per-app-redis-namespace.md)).
- **pnpm workspaces + catalog** — single-version dependency catalog so packages stay aligned.
- **Scaffolding** — `pnpm turbo gen` wires a new package/feature with configs, boundary tags, tRPC plumbing, and a compliant `acme` test-policy block already in place. Never hand-roll a package. It is for packages you are creating and never for a package taken from the bank, which arrives through `bank:sync` and is updated by it. Generate over one and you replace shared code with a fresh template, destroying the merge base with it.
...
- **One composite gate** — `pnpm quality-gate` is a **read-only, parallel** verify: one combined `turbo run` (lint + format + typecheck + build + test) alongside check:exports + check:imports + check:bank-paths + check:bank-tokens + check:adrs + check:portable + boundaries + workspace lint + dep lint + test-policy + gitleaks. The same checks CI runs. Auto-fix is a separate `pnpm tidy` (`lint:fix` + `format:fix`) run before the gate ([ADR 0020](adr/0020-commit-tidies-gate-verifies.md)).
...
**The daily loop.** `pnpm dev` is graph-aware — it brings up the Docker infra each target app's dependency closure needs, waits for health, pushes the schema, then runs the dev servers ([ADR 0009](../tooling/workspace-graph/docs/adr/0001-graph-derived-dev-infra.md)):
...
pnpm quality-gate            # then verify (read-only, parallel): build + turbo(lint+format+typecheck) + test + check:exports + check:imports + check:bank-paths + check:bank-tokens + check:adrs + check:portable + boundaries + lint:ws + deps:lint + test:policy + gitleaks
```
...
| I want to…                           | Do this                                                                                                                                         |
| ------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| Add a package or feature             | `pnpm turbo gen` — never hand-roll one, and never over a package taken from the bank                                                            |
| Eject one app into a standalone repo | `pnpm prune <app>` → self-contained tree in `out/`                                                                                              |
| Start a repo of my own from Trellis  | `pnpm prune <app>` for a Trellis app; `pnpm setup:wizard` when the app is yours ([three routes](bank.md#which-route-you-are-on))                |
| List tracked test-coverage gaps      | `pnpm test:policy --todos`                                                                                                                      |
| Add a shadcn UI component            | `pnpm ui-add`                                                                                                                                   |
| Swap LLM / embedding provider        | set `LLM_PROVIDER` / `EMBED_PROVIDER` in `.env` (see [@acme/models ADR 0001](../packages/shared/models/docs/adr/0001-multi-provider-models.md)) |
...
| Find imports nothing declares        | `pnpm check:imports`                                                                                                                            |
...
| ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm dev [app ...]`                                       | Graph-aware dev: bring up the infra the target apps need (or all apps), wait healthy, push schema, run servers ([ADR 0009](../tooling/workspace-graph/docs/adr/0001-graph-derived-dev-infra.md))                                                                                                     |
...
| `pnpm setup:wizard`                                        | **Consumer-side.** Author `bank.manifest.json` from a package selection — interactive picker on a TTY, `--upstream`/`--ref`/`--packages`/`--bundles` to skip it, `--list` to print the offer. Writes the manifest and nothing else ([the bank](bank.md#3-author-bankmanifestjson))                   |
| `pnpm bank:sync`                                           | **Consumer-side.** Rewrite `vendor/bank` to the bank subset pinned in `bank.manifest.json`; merges nothing ([the bank](bank.md))                                                                                                                                                                     |
| `pnpm bank:sync --check`                                   | **Consumer-side.** Report how far behind the pinned bank tag is and which vendored paths are locally modified. Writes nothing; exits `0` up to date, `1` error, `2` behind                                                                                                                           |
| `pnpm bank:contribute <path>`                              | **Consumer-side, never automated.** Diff a path against the bank, refuse anything outside `contributable`, scan with gitleaks, and open a PR after a typed confirmation ([the bank](bank.md#contributing-back-to-the-bank))                                                                          |
...
| `pnpm check:imports`                                       | Every external import is declared by the package that makes it — a gate stage, unlike `deps:check`                                                                                                                                                                                                   |
...
| `pnpm quality-gate`                                        | The full pre-push gate — **read-only, parallel**: build + turbo(lint+format+typecheck) + test + check:exports + check:imports + check:bank-paths + check:bank-tokens + check:adrs + check:portable + boundaries + test-policy + lint:ws + deps:lint + gitleaks                                       |
| `pnpm turbo gen`                                           | Scaffold a new package/feature. For packages you create, never for a package taken from the bank, which arrives and updates through `bank:sync` ([the bank](bank.md#which-route-you-are-on))                                                                                                         |
...
| `pnpm with-env <cmd>`                                      | Run a command with the infra env (`deploy/.env`) loaded (dotenv). Per-app application env lives in `apps/<app>/.env`, loaded by each app's own `with-env` (no shared root `.env` — [ADR 0029](../apps/docs/adr/0003-per-app-env-ownership.md))                                                       |
| `pnpm env:pull` / `pnpm env:push`                          | ⚠️ Sync local `.env` files with a pluggable secrets backend. `.env.example` is the contract (empty value = secret). Opt-in via `SECRETS_BACKEND` (no default): `localstack` (dev/demo vault) or `aws` (real cloud). See [ADR 0001](../tooling/secrets-sync/docs/adr/0001-pluggable-secrets-sync.md). |
...
- **Add or remove a feature from an app.** Edit the app's dependencies and its adapter wiring (mount the router, render the provider, drop the components). A client build with no billing, or with an extra bespoke feature, is _a new app importing a different subset_ — not a fork. The **slim apps are the worked proof**: they drop auth + billing entirely and inject a constant principal + `unlimitedEntitlements` ([ADR 0010](../apps/docs/adr/0001-slim-no-auth-apps.md)).
- **Extract an app from the monorepo.** `pnpm prune <app>` emits a standalone repo for one app — installs and builds with no monorepo. The slice contract is what makes this mechanical rather than a manual untangle. If the app is yours rather than Trellis's, `pnpm setup:wizard` is the other direction: it authors a bank selection your repo pulls the packages in through ([three routes](bank.md#which-route-you-are-on)).
- **Swap the LLM or embedding provider.** Set `LLM_PROVIDER` / `EMBED_PROVIDER` (chosen independently). Deleting a provider entirely is: delete its file, drop its `case` in `resolve.ts`, `pnpm remove` its SDK. See [@acme/models ADR 0001](../packages/shared/models/docs/adr/0001-multi-provider-models.md).
...
- **Better Auth for auth**, self-hosted — sessions are rows in the app's own Postgres, in a dedicated `auth` schema ([@acme/auth ADR 0001](../packages/shared/auth/docs/adr/0001-self-hosted-better-auth.md), [@acme/auth ADR 0002](../packages/shared/auth/docs/adr/0002-auth-tables-in-a-dedicated-schema.md)). Behind a framework seam and _droppable_: the slim apps run with no auth provider in the graph at all. It replaced a hosted identity provider, and **there is no migration path off that provider**: moving real identities and credentials is the deployment's own job.
