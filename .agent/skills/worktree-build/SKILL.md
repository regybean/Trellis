...
- **A fresh worktree has no `.env` (it's gitignored, so it never copies in) — expect app-level builds to fail env validation, and don't mistake it for your bug.** `pnpm typecheck` and `pnpm build` fan out through turbo `dependsOn` to the app's `next build`, which validates *runtime* env (Stripe keys, DB creds, etc.) and aborts with `❌ Invalid environment variables: [...]` listing vars you never touched. This is environmental, not a regression. Before chasing it: confirm the failing var is unrelated to your change (`grep` the error for your new keys — if none appear, it's pre-existing). To actually verify your work, scope checks to the packages you changed — `turbo run typecheck lint -F <pkg>...` — and rely on `pnpm boundaries` / `pnpm lint:ws` / `pnpm lint` (none of which need `.env`). Only a `next build` failure that names a var *you* added is yours to fix.
